...
CREATE POLICY "anonymous_sessions_all_operations" ON anonymous_sessions
  FOR ALL USING (true) WITH CHECK (true);
...
BEGIN
  ...
  BEGIN
    ...
    result := result || 'SUCCESS: Anonymous session created\n';
    ...
    result := result || 'SUCCESS: User created\n';
    
    -- 清理
    DELETE FROM users WHERE id = test_user_id;
    DELETE FROM anonymous_sessions WHERE id = test_session_id;
    result := result || 'SUCCESS: Cleanup completed\n';
