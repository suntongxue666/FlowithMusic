...
function HistoryContent() {
  ...
  const handleToggleVisibility = async (linkId: string, currentStatus: boolean | undefined) => {
    try {
      const newStatus = !currentStatus
      const success = await letterService.toggleLetterVisibility(linkId, newStatus)
      if (success) {
        setLetters(prev => prev.map(l => l.link_id === linkId ? { ...l, is_public: newStatus } : l))
        setToast({
          message: `Successfully\nLetter is now ${newStatus ? 'Public' : 'Private'}.`,
          isVisible: true
        })
      } else {
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            {isManageMode && (
                              <>
                                <button
                                  onClick={() => handleToggleVisibility(letter.link_id, letter.is_public)}
                                  style={{
                                    padding: '6px',
                                    color: '#666',
                                    background: 'none',
                                  ...
                                  title={letter.is_public ? "Make Private" : "Make Public"}
                                >
                                  {letter.is_public ? (
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
