...
export default function SpotifyEmbedPlayer({ track }: SpotifyEmbedPlayerProps) {
  ...
  const isApple = track.id.startsWith('apple-')
  const appleTrackId = isApple ? track.id.replace('apple-', '') : ''
  
  const embedUrl = isApple
    ? `https://embed.music.apple.com/us/album/song/${appleTrackId}`
    : isSafari && isMobile 
  ...
  if (showFallback || (isSafari && isMobile && !isApple)) {
    return (
        ...
        <div className="spotify-fallback">
          <div className="fallback-content">
            ...
            >
              ...
              {isApple ? '在 Apple Music 中播放' : '在 Spotify 中播放'}
            </button>
          </div>
        </div>
      </div>
